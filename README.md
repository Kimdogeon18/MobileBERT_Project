...
## 1. 프로젝트 개요
요즘 가짜뉴스가 사회적으로 큰 문제라고 느꼈고, 이를 어떻게든 걸러낼 수 있는 방법이 필요하다고 생각함. 사람이 일일이 뉴스 진위를 확인하는 건 한계가 있어서, 자동화된 판별 시스템이 필요하다고 판단함. 그래서 실제 뉴스 본문을 가지고 진짜인지 가짜인지 구분해주는 모델을 만들어보고자 함. 단순히 정확도를 높이는 데 그치지 않고, 데이터 구성이나 전처리가 성능에 얼마나 영향을 주는지도 직접 실험해봄. 결과적으로 신뢰도 높은 뉴스 필터링 모델을 만드는 데 필요한 기준을 고민해볼 수 있었음.
---
