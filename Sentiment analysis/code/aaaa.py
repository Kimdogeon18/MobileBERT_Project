...
# 데이터 불러오기
data_path = "dataset/processed_WELFake_Dataset27000.csv"
...
# 텍스트와 라벨 추출
df['text'] = df['text'].fillna("")  # NaN 값을 빈 문자열로 처리
...
inputs = tokenizer(data_x, truncation=True, max_length=512, add_special_tokens=True, padding="max_length", return_tensors="pt")
...
test_sampler = torch.utils.data.SequentialSampler(test_data)  # SequentialSampler로 변경
test_dataloader = torch.utils.data.DataLoader(test_data, sampler=test_sampler, batch_size=batch_size)
print("데이터셋 구축 완료")

# 사전 학습된 모델 불러오기
model = MobileBertForSequenceClassification.from_pretrained("mobilebert_fake_news_headline_detector1")
model.to(device)
model.eval()
